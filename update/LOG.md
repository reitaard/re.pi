...
- Cleaned obsolete remote release/update experiment refs: `repi/canonical`, `repi/cleanup-output`, `repi/final-output`, `repi/integrate-v0.82.1`, `repi/parity-output`, `sync/upstream-v0.82.1`, and tag `repi-v0.82.1-r1`. Preserved `repi/preserve-custom`, `agent-harness`, and `phase-4a-target-routing`.
- Merged `repi/preserve-custom` into `agent-harness` and pushed `agent-harness` at `417f9cd44770a542057c9b6046bf15bb4adb3e20`.
- VPS `/root/re.pi` dirty Spotipy worker changes were left out of the merge and preserved under `/root/repi-backups/repi-worktree-before-clean-agent-harness-20260726T190604Z`; `/root/re.pi` was replaced with a clean `agent-harness` checkout at `417f9cd44770a542057c9b6046bf15bb4adb3e20`.
- Removed old VPS Recode installs under `/opt/repi` (`v0.81.0` through `v0.81.3` plus old bridge residue). Current VPS Recode install remains `/opt/recode/0.81.4-repi.2.dev.27.9e818840` with private Node `/opt/node-v26.5.0-linux-x64`.
