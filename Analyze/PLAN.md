...
Track 4 is derived from [`analyze/COMPARE.md`](../analyze/COMPARE.md) and is intentionally broader than startup. C4 must re-rank it using executable evidence, but the known gaps remain explicit meanwhile.
...
21. V2-C matched startup evidence — active. Clean-source uncontrolled-cache configured/isolated RPC and TUI baselines plus isolated Maestro service-ready, one-session spawn and warm-attach artifacts are retained under `Analyze/evidence/v2-c-2026-08-02/`. Ten-session measurement currently fails closed at the production eight-live-session bound; resource sampling and exact matched jcode/upstream endpoints remain.
