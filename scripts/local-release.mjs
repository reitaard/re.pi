...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `recode-${platform}.zip` : `recode-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createRecodeShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "recode.cmd"))) {
			writeFileSync(join(installDirectory, "recode.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\recode.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "recode.ps1"), '& "$PSScriptRoot/node_modules/.bin/recode.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "recode.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\recode.exe" %*\r\n');
		writeFileSync(join(installDirectory, "recode.ps1"), '& "$PSScriptRoot/node_modules/.bin/recode.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "recode"), join(installDirectory, "recode"));
}
...
if (!options.skipInstall) {
	...
	createRecodeShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createRecodeShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `recode-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "recode.exe" : "recode")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "recode.cmd" : "recode")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "recode.cmd" : "recode")} --help`);
	}
}
