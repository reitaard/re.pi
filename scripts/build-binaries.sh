...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --minify --keep-names --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/recode.exe"
        bun build --compile --minify --keep-names --target=bun-$platform ../orchestrator/dist/cli.js --outfile "$OUTPUT_DIR/$platform/recode-maestro.exe"
    else
        bun build --compile --minify --keep-names --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/recode"
        bun build --compile --minify --keep-names --target=bun-$platform ../orchestrator/dist/cli.js --outfile "$OUTPUT_DIR/$platform/recode-maestro"
    fi
done
