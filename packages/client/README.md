...
```ts
import { PiClient, type ByteTransportFactory } from "@reitaard/repi-client";
...
```ts
import { PiClient } from "@reitaard/repi-client";
import { createUnixTransportFactory } from "@reitaard/repi-client/unix";
...
The `@reitaard/repi-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@reitaard/repi-client/unix` subpath.
