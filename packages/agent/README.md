...
Stateful agent with tool execution and event streaming. Built on `@reitaard/repi-ai`.

## Installation

```bash
npm install @reitaard/repi-agent-core
```
...
```typescript
declare module "@reitaard/repi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@reitaard/repi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@reitaard/repi-agent-core";
