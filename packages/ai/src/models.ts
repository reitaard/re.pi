...
class ModelsImpl implements MutableModels {
	...
	private async applyAuth<TOptions extends StreamOptions & ModelsStreamTransforms>(
		...
		const resolution = await resolveProviderAuth(this.requireProvider(model), this.credentials, this.authContext, {
		...
		let headers = mergeHeaders(mergeHeaders(auth.headers, model.headers), options?.headers);
		if (options?.transformHeaders) headers = await options.transformHeaders(headers ?? {});
		const env = resolution.env || options?.env ? { ...(resolution.env ?? {}), ...(options?.env ?? {}) } : undefined;
		const { transformHeaders: _transformHeaders, ...providerOptions } = options ?? {};
		let requestOptions: TOptions | undefined = { ...providerOptions, apiKey, headers, env } as TOptions;
