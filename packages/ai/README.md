...
```bash
npm install @reitaard/repi-ai
```

TypeBox exports are re-exported from `@reitaard/repi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@reitaard/repi-ai';
...
import { openaiProvider } from '@reitaard/repi-ai/providers/openai';
import { openrouterProvider } from '@reitaard/repi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@reitaard/repi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@reitaard/repi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@reitaard/repi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@reitaard/repi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@reitaard/repi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@reitaard/repi-ai';
...
```typescript
import { builtinImagesModels } from '@reitaard/repi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@reitaard/repi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@reitaard/repi-ai';
import { openAICompletionsApi } from '@reitaard/repi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@reitaard/repi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@reitaard/repi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@reitaard/repi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@reitaard/repi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@reitaard/repi-ai';
import { anthropicProvider } from '@reitaard/repi-ai/providers/anthropic';
import { openaiProvider } from '@reitaard/repi-ai/providers/openai';
import { googleProvider } from '@reitaard/repi-ai/providers/google';
...
- `@reitaard/repi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@reitaard/repi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@reitaard/repi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@reitaard/repi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@reitaard/repi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@reitaard/repi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@reitaard/repi-ai/bedrock-provider';
...
npx @reitaard/repi-ai list               # list available providers
```
...
The legacy flow functions remain available via the `@reitaard/repi-ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@reitaard/repi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@reitaard/repi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@reitaard/repi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@reitaard/repi-ai`.
