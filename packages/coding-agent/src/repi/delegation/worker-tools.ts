...
function formatTurn(turn: WorkerConversationTurnResult): string {
	...
	const identity = formatNamedWorkerIdentity({ displayName: result.workerName, aliases: result.workerAliases });
...
function formatStatus(snapshot: WorkerConversationSnapshot): string {
	...
	const output = snapshot.lastOutput
		...
		: "";
	const identity = formatNamedWorkerIdentity({ displayName: snapshot.workerName, aliases: snapshot.workerAliases });
...
export function createWorkerControlTools(directory: WorkerDirectory): AgentTool<any>[] {
	...
	const listTool: AgentTool<typeof listSchema, { workers: WorkerDescriptor[] }> = {
		...
		label: "worker_list",
		description:
			"List the live worker directory: canonical ids, display names, roles, personality, skill, tools, model, reasoning and token budget.",
		...
		async execute() {
			...
			const lines = workers.map((worker) => {
				...
				const model = worker.modelPreference
					? ` model=${worker.modelPreference.provider}/${worker.modelPreference.id}`
					: " model=parent";
				return `- id=${worker.id}; name=${formatNamedWorkerIdentity(worker)}; role=${worker.description}; tools=${worker.tools.join(",")}${skill}${personality}; thinking=${worker.thinkingLevel}; maxTokens=${worker.maxOutputTokens};${model}`;
			});
	...
	const startTool: AgentTool<typeof startSchema, WorkerConversationTurnResult> = {
		...
		label: "worker_start",
		description:
			"Open a named worker conversation and send its first message. Reuse the returned full conversationId verbatim with worker_message/status/cancel/close. Never automatically retry a failed worker or replace it with parent work.",
	...
	const messageTool: AgentTool<typeof messageSchema, WorkerConversationTurnResult> = {
		...
		description:
			"Continue an existing worker conversation using the exact conversationId returned by worker_start. Never automatically retry a failed turn or replace it with parent work.",
	...
	const statusTool: AgentTool<typeof statusSchema, { conversations: WorkerConversationSnapshot[] }> = {
		...
		label: "worker_status",
		description:
			"Show live or recent worker conversation state: full id, identity, run, status, elapsed time, turns, tool and bounded result. Status never retries work.",
	...
	const cancelTool: AgentTool<typeof conversationSchema, { cancelled: boolean }> = {
		name: "worker_cancel",
		label: "worker_cancel",
		description: "Cancel the currently running turn for one worker conversation without closing it.",
