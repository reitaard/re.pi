...
export class WorkerDirectory {
	...
	private recordResult(record: WorkerConversationRecord, message: string, result: NamedWorkerRunResult): void {
		...
		record.history.push({ role: "worker", text: result.output || result.error || `[${result.status}]` });
		this.trimHistory(record);
	}
	...
	private async run(
		...
		return runNamedWorker({
			cwd: this.cwd,
			worker: { ...worker, thinkingLevel: settings.thinkingLevel, maxOutputTokens: settings.maxOutputTokens },
			model,
