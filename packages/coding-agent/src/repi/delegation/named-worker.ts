...
import { createFindTool, createGrepTool, createLsTool, createReadTool } from "../../core/tools/index.ts";
...
export type NamedWorkerProgressEvent =
	| { type: "start"; runId: string; workerId: string; workerName: string }
	| {
...
function createWorkerTools(
	...
	return names.map((name) => {
		switch (name) {
			case "read":
				return createReadTool(cwd) as AgentTool;
			case "grep":
				return createGrepTool(cwd) as AgentTool;
			case "find":
				return createFindTool(cwd) as AgentTool;
			case "ls":
				return createLsTool(cwd) as AgentTool;
			case "web_search":
			case "fetch_content":
...
function clipResult(text: string, limit: number): { output: string; truncated: boolean } {
	...
	return { output: `${text.slice(0, limit - suffix.length)}${suffix}`, truncated: true };
}
...
function buildWorkerSystemPrompt(worker: NamedWorkerDefinition, cwd: string): string {
	...
	const hasWorkspaceTools = toolNames.some((name) => ["read", "grep", "find", "ls"].includes(name));
...
function emitProgress(options: RunNamedWorkerOptions, event: NamedWorkerProgressEvent): void {
	try {
		...
		// Progress observers never break worker execution.
	}
}
