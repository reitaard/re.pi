...
function getMaestroEndpoint(): string {
	const baseDir =
		process.env.PI_ORCHESTRATOR_DIR ?? join(process.env.PI_CONFIG_DIR ?? join(homedir(), ".pi"), "orchestrator");
...
function parseHealth(line: string): MaestroHealthWire | undefined {
	...
	return health as unknown as MaestroHealthWire;
}

async function queryHealth(): Promise<MaestroHealthWire | undefined> {
	return await new Promise<MaestroHealthWire | undefined>((resolvePromise) => {
		const socket = createConnection(getMaestroEndpoint());
		let buffer = "";
		let settled = false;
		const finish = (health?: MaestroHealthWire): void => {
			if (settled) return;
			settled = true;
			clearTimeout(timer);
		...
		timer.unref();
		socket.once("connect", () => socket.write('{"type":"health"}\n'));
...
function formatStatus(health: MaestroHealthWire): string {
