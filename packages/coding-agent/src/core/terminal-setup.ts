...
export function formatTerminalKeyboardReport(terminal: Terminal): string {
	...
	if (status.protocol === "legacy" || labels.some(([binding]) => !status.confirmed[binding])) {
		lines.push(
			"A terminal that cannot send distinct modified keys may not support every RePi binding. Run /tui-setup apply windows-terminal or /tui-setup apply vscode when applicable.",
		);
	}
	return lines.join("\n");
}
