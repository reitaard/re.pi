...
import type { ImageContent, Model, Provider, ProviderHeaders } from "@reitaard/repi-ai";
...
export class ExtensionRunner {
	...
	bindCore(
		...
		for (const { provider, extensionPath } of this.runtime.pendingNativeProviderRegistrations) {
			try {
				if (providerActions?.registerNativeProvider) providerActions.registerNativeProvider(provider);
				else this.modelRegistry.registerProvider(provider);
			} catch (err) {
				this.emitError({
					extensionPath,
					event: "register_provider",
