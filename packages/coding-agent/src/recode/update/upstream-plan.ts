...
const OWNERSHIP_PATH = join("repi", "upstream-ownership.json");
...
function isRecodeRoot(directory: string): boolean {
	const productPath = join(directory, "repi", "product.json");
	...
	try {
		...
		return isRecord(product) && product.productName === "RePi" && product.appName === "recode";
	} catch {
		return false;
	}
}
...
function upstreamUsage(): void {
	...
	console.log("  target  Local Git revision to compare (default from repi/upstream-ownership.json)");
}
