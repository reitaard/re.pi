...
import { repiOpenProvider } from "../src/repi/providers/open-provider.ts";

describe("RePi Open Provider", () => {
	...
	it("configures a fresh user and imports LM Studio capabilities", async () => {
		agentDir = await mkdtemp(join(tmpdir(), "repi-open-provider-"));
		...
		const fetchMock = vi.fn(async (input: string | URL | Request) => {
			...
			return new Response(JSON.stringify({ data: [{ id: "qwen3.5-9b" }, { id: "text-embedding-model" }] }));
		});
		vi.stubGlobal("fetch", fetchMock);

		await repiOpenProvider(pi);
