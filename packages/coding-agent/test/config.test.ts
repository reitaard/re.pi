...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@reitaard/repi-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @reitaard/repi-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getUpdateInstruction("@reitaard/repi-coding-agent")).toBe(
			"Update @reitaard/repi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		expect(command).toEqual({
			...
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @reitaard/repi-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@reitaard/repi-coding-agent", undefined, {
			packageName: "@reitaard/repi-coding-agent",
			installSpec: "@reitaard/repi-coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@reitaard/repi-coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @reitaard/repi-coding-agent@1.2.3`,
		});
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @reitaard/repi-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@reitaard/repi-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @reitaard/repi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@reitaard/repi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @reitaard/repi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@reitaard/repi-coding-agent";
