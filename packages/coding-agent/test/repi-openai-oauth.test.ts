...
import { repiOpenAIOAuth } from "../src/repi/providers/openai-oauth.ts";

describe("RePi OpenAI OAuth provider", () => {
	const originalRecodeBaseUrl = process.env.RECODE_OPENAI_OAUTH_BASE_URL;
	const originalRepiBaseUrl = process.env.REPI_OPENAI_OAUTH_BASE_URL;

	afterEach(() => {
		if (originalRecodeBaseUrl === undefined) {
			delete process.env.RECODE_OPENAI_OAUTH_BASE_URL;
		} else {
			process.env.RECODE_OPENAI_OAUTH_BASE_URL = originalRecodeBaseUrl;
		}
		if (originalRepiBaseUrl === undefined) {
			delete process.env.REPI_OPENAI_OAUTH_BASE_URL;
		} else {
			process.env.REPI_OPENAI_OAUTH_BASE_URL = originalRepiBaseUrl;
		}
		vi.unstubAllGlobals();
	});
	...
	it("keeps startup optional when the local proxy is stopped", async () => {
		...
		await expect(repiOpenAIOAuth(pi)).resolves.toBeUndefined();
